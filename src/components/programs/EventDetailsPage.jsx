...
Shows the detail inpormation about each specific event by its id
*/
...
const EventDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        setLoading(true);
        const data = await getEventById(eventId); // ✅ Use the service
        setEvent(data);
      } catch (err) {
        console.error(err);
  ...
  // Helper: Render external links with copy option
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-slate-100 via-slate-300 to-gray-700 text-white font-sans animate-gradient-x bg-[length:200%_200%] overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 md:flex md:space-x-12 items-start">
        ...
        <section className="bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-700 flex-1">
          ...
          <div className="space-y-5 text-gray-300">
            ...
            />
            <InfoItem icon={<FaMapMarkerAlt />} text={event.location} />
            <InfoItem icon={<FaGlobe />} text={event.mode} />
            <InfoItem
              icon={<FaUsers />}
            ...
            <InfoItem
              icon={<FaUserFriends />}
              text={`Speakers: ${event.speakers.join(", ")}`}
            />
            <InfoItem
              icon={<FaTags />}
              text={`Price: ${event.isFree ? "Free" : `₹${event.price}`}`}
            />

            {event.meetingLink &&
              renderExternalLink(event.meetingLink, "Join Meeting")}
            {event.registrationLink &&
              renderExternalLink(event.registrationLink, "Register")}
            {event.feedbackFormLink &&
              event.feedbackFormLink !== "true" &&
              renderExternalLink(event.feedbackFormLink, "Provide Feedback")}
          </div>
...
const InfoItem = ({ icon, text, status }) => (
  <div className="flex items-center space-x-3">
    <div className="text-pink-500 mt-1">{icon}</div>
    <span className={`text-gray-300 ${status ? getStatusColor(status) : ""}`}>
      {text}
    </span>
  </div>
);
